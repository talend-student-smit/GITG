 ...
 * 
 * @author SJ, Smitha
 * @version 7.3.1.20210318_0728-patch
 * @status
 */
public class ragh implements TalendJob {
	static {
		System.setProperty("TalendJob.log", "ragh.log");
	}

	private static org.apache.logging.log4j.Logger log = org.apache.logging.log4j.LogManager.getLogger(ragh.class);
	...
	private final String jobName = "ragh";
	private final String projectName = "TEST_JENKINS";
				...
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(ragh.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class ContextBean {
		static String evaluate(String context, String contextExpression)
			...
			java.io.InputStream inContext = ragh.class.getClassLoader()
					.getResourceAsStream("test_jenkins/ragh_0_1/contexts/" + context + ".properties");
			if (inContext == null) {
				inContext = ragh.class.getClassLoader()
	...
	public static class row1Struct implements routines.system.IPersistableRow<row1Struct> {
		final static byte[] commonByteArrayLock_TEST_JENKINS_ragh = new byte[0];
		static byte[] commonByteArray_TEST_JENKINS_ragh = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_TEST_JENKINS_ragh.length) {
					if (length < 1024 && commonByteArray_TEST_JENKINS_ragh.length == 0) {
						commonByteArray_TEST_JENKINS_ragh = new byte[1024];
					} else {
						commonByteArray_TEST_JENKINS_ragh = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_TEST_JENKINS_ragh, 0, length);
				strReturn = new String(commonByteArray_TEST_JENKINS_ragh, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_TEST_JENKINS_ragh) {
...
///////////////////////    			
	...
	public static void main(String[] args) {
		final ragh raghClass = new ragh();

		int exitCode = raghClass.runJobInTOS(args);
		if (exitCode == 0) {
			log.info("TalendJob: 'ragh' - Done.");
		}
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = ragh.class.getClassLoader()
					.getResourceAsStream("test_jenkins/ragh_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = ragh.class.getClassLoader()
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : ragh");
		}
		if (enableLogStash) {
...
/************************************************************************************************
 * 52074 characters generated by Talend Cloud Real-Time Big Data Platform on the
 * 11 June, 2021 9:13:08 PM IST
 ************************************************************************************************/